 ...
 * 
 */
public class PhotoFlipView extends View {
    ...
    public ArrayList<Bitmap> getmPages() {
	return mPages;
    }

    public void setmPages(ArrayList<Bitmap> mPages) {
	this.mPages = mPages;
    }
	...
	public void handleMessage(Message msg) {
	    PhotoFlipView.this.FlipAnimationStep();
	}
     ...
     * @param context
     */
    public PhotoFlipView(Context context) {
	super(context);
	init(context);
	ResetClipEdge();
    }
     ...
     */
    public PhotoFlipView(Context context, AttributeSet attrs) {
	super(context, attrs);
	init(context);
	...
	// Set the default props, those come from an XML :
	// set curl speed WELL TESTED

	/*
	 * mCurlSpeed = 30; mUpdateRate = 33; mInitialEdgeOffset = 1; mCurlMode
	 * = 1;
	 */
	...
	mUpdateRate = 63;
	mInitialEdgeOffset = 1;
	mCurlMode = 1;

	// LEGACY PAGE HANDLING!

	// Create pages by taking photos from DataBase BLOG
	// %%%%%%%%%%%%%%%%%%%%%%%%%%%5
	...
	if (mPages != null && mPages.size() > 0
		&& mPages.get(foreground) != null)
	    mForeground = mPages.get(foreground);

	if (mPages != null && mPages.size() > 1
		&& mPages.get(background) != null)
